...
export async function findAll(transaction, { page = 1, limit = 50, filters = {}, range = {}, sortBy = 'number', sortOrder = 'DESC' }) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      ...
      if (filters.typeHash) {
        const type = await typeRepository.findOne(t, {
      ...
      const { rows, count } = await solicitationRepository.findAll(t, {
...
export async function findOne({ db, transaction } = {}, id) {
  try {
    ...
    const execute = async (t) => {
      ...
      return ServiceResponse.success(solicitation)
    })
  } catch (error) {
    return ServiceResponse.error(error)
  }
...
export async function create(transaction, data) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      ...
      const result = await solicitationRepository.create(t, {
        ...finalData,
        companyId: session.company.id,
...
export async function update(transaction, id, data) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      const existing = await solicitationRepository.findOne(t, {
        attributes: ['id'],
      ...
      const finalData = sanitize(data)
      await solicitationRepository.update(t, { where: { id } }, finalData)
...
export async function toStatus(transaction, id, statusId, description = null) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      ...
      await solicitationRepository.update(t, { where: { id } }, updateData)
      return ServiceResponse.success({ id })
    })
  } catch (error) {
    return ServiceResponse.error(error)
...
export async function destroy(transaction, id) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      ...
      await solicitationRepository.destroy(t, { where: { id } })
      return ServiceResponse.success({ id })
    })
  } catch (error) {
    return ServiceResponse.error(error)
...
export async function saveDocuments(transaction, solicitationId, documents = []) {
  try {
    ...
    return await db.withTransaction(transaction, async (t) => {
      ...
      for (const doc of documents) {
        ...
        if (doc.id) {
          const result = await documentService.update(t, doc.id, payload)
          if (result.header.status !== ServiceStatus.SUCCESS) throw new Error(result.body.message)
        } else {
          ...
          documentService.transmit(t, doc.id)
        }
      ...
      if (newDocIds.length > 0) {
        const result = await documentService.generateFinance(t, newDocIds, solicitation.payments)
        if (result.header.status !== ServiceStatus.SUCCESS) throw new Error(result.body.message)
      }
...
export async function generateDocuments(transaction, solicitationIds = []) {
  ...
  try {
    return await db.withTransaction(transaction, async (t) => {
      ...
      const docTypes = await db.DocumentType.findAll({ attributes: ['id', 'initials'], transaction: t })
